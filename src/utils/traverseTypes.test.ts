...
import { TypeNode, getExtractedTypeNames, isTypeNode } from "./traverseTypes";
...
function extractNames(sourceText: string) {
  ...
  const typeNameMapBuilder = (node: ts.Node) => {
    if (isTypeNode(node)) {
      mapping.set(node.name.text, node);
    }
  };

  ts.forEachChild(sourceFile, typeNameMapBuilder);

  return getExtractedTypeNames(declaration, sourceFile, mapping);
}
